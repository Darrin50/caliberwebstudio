  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <p className="mb-6 text-sm text-navy/60">
        Choose a window, then pick your 1-hour start time.
      </p>
                ...
                {isExpanded && (
                  <div className="grid grid-cols-2 gap-2 px-5 pb-5 sm:grid-cols-4">
                    {windowSlots.map(({ slot, avail }) => {
                      ...
                      const isSelected =
                        state.timeWindow === window.id && state.startTime === slot
                        ...
                        >
                          ...
                          <span
                            className={`mt-1 text-xs ${
                              isSelected ? 'text-white/80' : isFull ? 'text-red-500' : 'text-navy/50'
                            }`}
                          >
                            {isFull
                              ? 'Full'
                              : `${avail.available} spot${avail.available === 1 ? '' : 's'} left`}
                          </span>
                        </button>
                      )
                    })}
