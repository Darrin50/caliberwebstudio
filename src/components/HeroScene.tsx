'use client';

import { useEffect, useRef } from 'react';

declare const THREE: any;

export default function HeroScene() {
  const rendererRef = useRef<any>(null);
  const frameRef    = useRef<number>(0);
  const visibleRef  = useRef<boolean>(true);

  useEffect(() => {
    if (typeof window === 'undefined') return;

    // ── Reduced-motion gate ──────────────────────────────────────────
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

    // ── Mobile gate (< 768px) ────────────────────────────────────────
    if (window.innerWidth < 768) return;

    const init = () => {
      const loadThree = (): Promise<void> =>
        new Promise((resolve) => {
          if ((window as any).THREE) { resolve(); return; }
          const script = document.createElement('script');
          script.src = 'https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js';
          script.async = true;
          script.onload = () => resolve();
          document.head.appendChild(script);
        });

      loadThree().then(() => {
        const canvas = document.getElementById('hero-canvas') as HTMLCanvasElement;
        if (!canvas) return;

        const THREE = (window as any).THREE;

        // ── Renderer — capped DPR ──
        const renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: true });
        renderer.setSize(window.innerWidth, window.innerHeight);
        renderer.setClearColor(0x000000, 0);
        renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // was 2
        rendererRef.current = renderer;

        // ── Camera ──
        const camera = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 0.1, 1000);
        camera.position.z = 24;

        // ── Scene ──
        const scene = new THREE.Scene();
        const sceneGroup = new THREE.Group();
        scene.add(sceneGroup);

        const raycaster = new THREE.Raycaster();
        const mouse     = new THREE.Vector2();
        const interactables: any[] = [];

        // ── Globe — reduced segments (was 28×20) ──
        const globeGroup = new THREE.Group();
        globeGroup.position.set(-10, 1, -2);
        sceneGroup.add(globeGroup);

        const globeGeo = new THREE.SphereGeometry(5, 20, 14);
        const globeMat = new THREE.MeshBasicMaterial({
          color: 0x1e3d8f, wireframe: true, transparent: true, opacity: 0.3,
        });
        const globe = new THREE.Mesh(globeGeo, globeMat);
        globeGroup.add(globe);
        interactables.push(globe);

        const globeInnerGeo = new THREE.SphereGeometry(4.85, 20, 14);
        const globeInnerMat = new THREE.MeshBasicMaterial({ color: 0x0a1a3f, transparent: true, opacity: 0.15 });
        globeGroup.add(new THREE.Mesh(globeInnerGeo, globeInnerMat));

        // Latitude rings — reduced count (5→3)
        [-0.75, 0, 0.75].forEach((lat) => {
          const radius = Math.cos(lat * 0.6) * 5.1;
          const ringGeo = new THREE.RingGeometry(radius - 0.02, radius + 0.02, 48);
          const ringMat = new THREE.MeshBasicMaterial({
            color: 0x3a6fd8, transparent: true, opacity: 0.12, side: THREE.DoubleSide,
          });
          const ring = new THREE.Mesh(ringGeo, ringMat);
          ring.position.y = Math.sin(lat * 0.6) * 5.1;
          ring.rotation.x = Math.PI / 2;
          globeGroup.add(ring);
        });

        const glowRingGeo = new THREE.RingGeometry(5.3, 5.8, 48);
        const glowRingMat = new THREE.MeshBasicMaterial({
          color: 0x1e3d8f, transparent: true, opacity: 0.06, side: THREE.DoubleSide,
        });
        const glowRing = new THREE.Mesh(glowRingGeo, glowRingMat);
        globeGroup.add(glowRing);

        // Data points — reduced to 8 (was 12)
        const dataPoints: any[] = [];
        const pointPositions = [
          [0.3, 0.8, 0.5], [-0.6, 0.4, 0.7], [0.7, -0.2, 0.7],
          [-0.4, -0.6, 0.7], [0.1, 0.5, -0.9], [-0.8, 0.1, -0.6],
          [0.5, -0.8, 0.3], [-0.3, 0.9, -0.3],
        ];
        pointPositions.forEach((p) => {
          const len = Math.sqrt(p[0] ** 2 + p[1] ** 2 + p[2] ** 2);
          const nx = (p[0] / len) * 5.05;
          const ny = (p[1] / len) * 5.05;
          const nz = (p[2] / len) * 5.05;
          const dot = new THREE.Mesh(
            new THREE.SphereGeometry(0.08, 6, 6),
            new THREE.MeshBasicMaterial({ color: 0x5a8fff })
          );
          dot.position.set(nx, ny, nz);
          globeGroup.add(dot);
          dataPoints.push({ mesh: dot, phase: Math.random() * Math.PI * 2 });
        });

        // Connection arcs — 5 (was 7)
        [[0, 1], [2, 4], [3, 5], [6, 7], [1, 6]].forEach(([a, b]) => {
          const pa = dataPoints[a].mesh.position;
          const pb = dataPoints[b].mesh.position;
          const mid = new THREE.Vector3().addVectors(pa, pb).multiplyScalar(0.5);
          mid.normalize().multiplyScalar(6.5);
          const curve = new THREE.QuadraticBezierCurve3(pa.clone(), mid, pb.clone());
          const arcGeo = new THREE.BufferGeometry().setFromPoints(curve.getPoints(16));
          const arcMat = new THREE.LineBasicMaterial({ color: 0x3a6fd8, transparent: true, opacity: 0.15 });
          globeGroup.add(new THREE.Line(arcGeo, arcMat));
        });

        // ── Floating spheres — 6 (was 8) ──
        const sphereConfigs = [
          { x:  9, y:  3, z:  4, r: 1.1,  color: 0x4d8fff, glowColor: 0x7ab4ff },
          { x:  6, y: -4, z:  8, r: 0.75, color: 0x3a6fd8, glowColor: 0x5a9aff },
          { x: -5, y:  6, z:  5, r: 0.90, color: 0x5a8fff, glowColor: 0x88bbff },
          { x: 12, y: -1, z:  3, r: 0.60, color: 0x7ab4ff, glowColor: 0x99ccff },
          { x:  4, y:  8, z: -2, r: 0.80, color: 0x4d8fff, glowColor: 0x7ab4ff },
          { x: -7, y: -3, z:  2, r: 0.65, color: 0x5a8fff, glowColor: 0x88bbff },
        ];
        const spheres = sphereConfigs.map((cfg) => {
          const glowMat  = new THREE.MeshBasicMaterial({ color: cfg.glowColor, transparent: true, opacity: 0.12, blending: THREE.AdditiveBlending, depthWrite: false });
          const midMat   = new THREE.MeshBasicMaterial({ color: cfg.glowColor, transparent: true, opacity: 0.18, blending: THREE.AdditiveBlending, depthWrite: false });
          const innerMat = new THREE.MeshBasicMaterial({ color: cfg.color,     transparent: true, opacity: 0.35, blending: THREE.AdditiveBlending, depthWrite: false });
          const group = new THREE.Group();
          group.add(new THREE.Mesh(new THREE.SphereGeometry(cfg.r * 2.2, 12, 12), glowMat));
          group.add(new THREE.Mesh(new THREE.SphereGeometry(cfg.r * 1.4, 12, 12), midMat));
          group.add(new THREE.Mesh(new THREE.SphereGeometry(cfg.r,       16, 16), innerMat));
          group.position.set(cfg.x, cfg.y, cfg.z);
          sceneGroup.add(group);
          return { group, glowMat, midMat, innerMat, originX: cfg.x, originY: cfg.y, originZ: cfg.z, phase: Math.random() * Math.PI * 2, speed: 0.3 + Math.random() * 0.4, driftRadius: 0.8 + Math.random() * 0.6 };
        });

        // ── Particles — 800 (was 1600) ──
        const particlesGeo = new THREE.BufferGeometry();
        const pos = new Float32Array(800 * 3);
        for (let i = 0; i < 800 * 3; i += 3) {
          pos[i]     = (Math.random() - 0.5) * 90;
          pos[i + 1] = (Math.random() - 0.5) * 60;
          pos[i + 2] = (Math.random() - 0.5) * 60;
        }
        particlesGeo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const particlesMat = new THREE.PointsMaterial({ color: 0xa8b8c8, size: 0.06, transparent: true, opacity: 0.45, sizeAttenuation: true });
        particlesMat.blending = THREE.AdditiveBlending;
        const particles = new THREE.Points(particlesGeo, particlesMat);
        sceneGroup.add(particles);

        const grid = new THREE.GridHelper(80, 24, 0x1e3d8f, 0x1e3d8f);
        grid.material.transparent = true;
        grid.material.opacity = 0.06;
        grid.position.y = -10;
        sceneGroup.add(grid);

        // ── Interaction state ──
        let isDragging = false, dragStarted = false, touchScrolling = false;
        let pointerDownX = 0, pointerDownY = 0, prevMouseX = 0, prevMouseY = 0;
        let targetCamX = 0, targetCamY = 0;
        let globeRotVelX = 0, globeRotVelY = 0.003;
        let sceneRotVelX = 0, sceneRotVelY = 0;
        let hoveredObj: any = null;

        const onPointerDown = (e: PointerEvent) => {
          pointerDownX = e.clientX; pointerDownY = e.clientY;
          prevMouseX   = e.clientX; prevMouseY   = e.clientY;
          dragStarted = isDragging = touchScrolling = false;
        };
        const onPointerMove = (e: PointerEvent) => {
          const rect = canvas.getBoundingClientRect();
          mouse.x =  ((e.clientX - rect.left) / rect.width)  * 2 - 1;
          mouse.y = -((e.clientY - rect.top)  / rect.height) * 2 + 1;
          targetCamX = (e.clientX / window.innerWidth  - 0.5) * 2;
          targetCamY = (e.clientY / window.innerHeight - 0.5) * 2;
          const held = e.buttons > 0;
          if (held) {
            const dx = e.clientX - pointerDownX, dy = e.clientY - pointerDownY;
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (!dragStarted && !touchScrolling && dist > 5) {
              if (e.pointerType === 'touch' && Math.abs(dy) > Math.abs(dx) * 1.4) {
                touchScrolling = true;
              } else {
                dragStarted = isDragging = true;
                canvas.style.cursor = 'grabbing';
                try { canvas.setPointerCapture(e.pointerId); } catch { /* ignore */ }
              }
            }
            if (touchScrolling) { prevMouseX = e.clientX; prevMouseY = e.clientY; return; }
            if (isDragging) {
              sceneRotVelY = (e.clientX - prevMouseX) * 0.005;
              sceneRotVelX = (e.clientY - prevMouseY) * 0.003;
              sceneGroup.rotation.y += sceneRotVelY;
              sceneGroup.rotation.x = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, sceneGroup.rotation.x + sceneRotVelX));
              e.preventDefault();
            }
          } else {
            if (isDragging || dragStarted) { isDragging = dragStarted = touchScrolling = false; canvas.style.cursor = 'grab'; }
            raycaster.setFromCamera(mouse, camera);
            const hits = raycaster.intersectObjects(interactables, false);
            canvas.style.cursor = hits.length > 0 ? 'grab' : 'grab';
            hoveredObj = hits.length > 0 ? hits[0].object : null;
          }
          prevMouseX = e.clientX; prevMouseY = e.clientY;
        };
        const onPointerUp = (e: PointerEvent) => {
          isDragging = dragStarted = touchScrolling = false;
          canvas.style.cursor = 'grab';
          try { canvas.releasePointerCapture(e.pointerId); } catch { /* ignore */ }
        };
        canvas.addEventListener('pointerdown',   onPointerDown);
        canvas.addEventListener('pointermove',   onPointerMove);
        canvas.addEventListener('pointerup',     onPointerUp);
        canvas.addEventListener('pointercancel', onPointerUp);

        // Scroll
        let lastScrollY = 0;
        const onScroll = () => {
          const scrollY = window.scrollY;
          const scrollProgress = scrollY / (document.documentElement.scrollHeight - window.innerHeight);
          camera.position.z = 24 + scrollProgress * 14;
          sceneGroup.rotation.z = scrollProgress * 0.08;
          const delta = scrollY - lastScrollY;
          sceneRotVelY   += delta * 0.00018;
          globeRotVelY   += delta * 0.00012;
          lastScrollY = scrollY;
          const translateY = scrollY * 0.3;
          const scale = Math.max(0.82, 1 - scrollY * 0.00025);
          canvas.style.transform = `translateY(${translateY}px) scale(${scale})`;
          canvas.style.transformOrigin = 'center top';
        };
        window.addEventListener('scroll', onScroll);

        // Resize
        const onResize = () => {
          camera.aspect = window.innerWidth / window.innerHeight;
          camera.updateProjectionMatrix();
          renderer.setSize(window.innerWidth, window.innerHeight);
        };
        window.addEventListener('resize', onResize);

        // ── Visibility observer — pause loop when hero is off-screen ──
        const canvasWrapper = canvas.parentElement;
        if (canvasWrapper) {
          const io = new IntersectionObserver(
            (entries) => { visibleRef.current = entries[0].isIntersecting; },
            { threshold: 0.01 }
          );
          io.observe(canvasWrapper);
          // cleanup stored on renderer for disposal
          (renderer as any).__io = io;
        }

        // ── Animation loop ──
        let frameCount = 0;
        const animate = () => {
          frameRef.current = requestAnimationFrame(animate);

          // Pause rendering when hero is scrolled out of view
          if (!visibleRef.current) return;

          frameCount++;

          if (!isDragging) {
            sceneRotVelX *= 0.93; sceneRotVelY *= 0.93;
            sceneGroup.rotation.y += sceneRotVelY;
            sceneGroup.rotation.x = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, sceneGroup.rotation.x + sceneRotVelX));
          }

          globeRotVelX *= 0.98; globeRotVelY *= 0.98;
          globeRotVelY += (0.003 - globeRotVelY) * 0.005;
          globeGroup.rotation.y += globeRotVelY;
          globeGroup.rotation.x += globeRotVelX;
          glowRing.rotation.z   += 0.002;

          dataPoints.forEach((dp) => {
            const pulse = 0.7 + Math.sin(frameCount * 0.04 + dp.phase) * 0.3;
            dp.mesh.scale.setScalar(pulse);
            dp.mesh.material.opacity = 0.5 + Math.sin(frameCount * 0.03 + dp.phase) * 0.5;
          });

          spheres.forEach((s) => {
            const t = frameCount * 0.008 * s.speed + s.phase;
            s.group.position.x = s.originX + Math.sin(t) * s.driftRadius;
            s.group.position.y = s.originY + Math.cos(t * 0.7) * s.driftRadius * 0.8;
            s.group.position.z = s.originZ + Math.sin(t * 0.5) * s.driftRadius * 0.5;
            s.group.scale.setScalar(0.88 + Math.sin(t * 1.8) * 0.12);
            s.glowMat.opacity  = 0.08 + Math.sin(t * 1.4) * 0.04;
            s.midMat.opacity   = 0.14 + Math.sin(t * 1.2) * 0.04;
            s.innerMat.opacity = 0.28 + Math.sin(t * 1.2) * 0.07;
          });

          particles.rotation.y += 0.0003;
          grid.position.z       += 0.03;

          camera.position.x += (targetCamX * 1.0 - camera.position.x) * 0.02;
          camera.position.y += (-targetCamY * 0.6 - camera.position.y) * 0.02;

          renderer.render(scene, camera);
        };
        animate();

        return () => {
          cancelAnimationFrame(frameRef.current);
          window.removeEventListener('scroll', onScroll);
          window.removeEventListener('resize', onResize);
          canvas.removeEventListener('pointerdown',   onPointerDown);
          canvas.removeEventListener('pointermove',   onPointerMove);
          canvas.removeEventListener('pointerup',     onPointerUp);
          canvas.removeEventListener('pointercancel', onPointerUp);
          if ((renderer as any).__io) (renderer as any).__io.disconnect();
          renderer.dispose();
        };
      });
    };

    // ── Mount after first paint (requestIdleCallback with setTimeout fallback) ──
    let cleanup: (() => void) | undefined;
    const scheduleInit = () => {
      // Use a Promise to capture the cleanup fn from the async init
      init();
    };

    if ('requestIdleCallback' in window) {
      const id = (window as any).requestIdleCallback(scheduleInit, { timeout: 2000 });
      return () => {
        (window as any).cancelIdleCallback(id);
        cleanup?.();
        if (rendererRef.current) rendererRef.current.dispose();
        cancelAnimationFrame(frameRef.current);
      };
    } else {
      const tid = setTimeout(scheduleInit, 100);
      return () => {
        clearTimeout(tid);
        cleanup?.();
        if (rendererRef.current) rendererRef.current.dispose();
        cancelAnimationFrame(frameRef.current);
      };
    }
  }, []);

  return (
    <canvas
      id="hero-canvas"
      style={{
        position: 'absolute',
        inset: 0,
        zIndex: 3,
        pointerEvents: 'auto',
        touchAction: 'pan-y',
        cursor: 'grab',
        width: '100%',
        height: '100%',
      }}
    />
  );
}
