...
export default function Nav() {
  ...
  return (
    <>
      <nav className="site-nav" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 clamp(16px, 4vw, 40px)', height: '72px', background: 'var(--nav-bg)', backdropFilter: 'blur(20px) saturate(180%)', WebkitBackdropFilter: 'blur(20px) saturate(180%)', borderBottom: '1px solid var(--border)', transition: 'background 0.4s ease' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <button ref={toggleRef} id="themeToggle" title="Toggle theme" style={{ width: '52px', height: '28px', borderRadius: '14px', border: 'none', background: 'rgba(168,184,200,0.15)', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '3px', transition: 'background 0.3s' }}>
            <span className="toggle-icon" style={{ width: '22px', height: '22px', borderRadius: '50%', background: 'linear-gradient(135deg, #ffcc40, #ff8800)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', transition: 'transform 0.3s', transform: 'translateX(24px)' }}>{'\u{1F319}'}</span>
          </button>
      ...
      <style>{`
        :root { --nav-bg: rgba(14,14,14,0.85); --hamburger-color: #D0D8E0; }
        [data-theme="light"] { --nav-bg: rgba(246,247,251,0.88); --hamburger-color: #1E3D8F; }
        .nav-hamburger span { background: #D0D8E0 !important; }
        [data-theme="light"] .nav-hamburger span { background: #1E3D8F !important; }
